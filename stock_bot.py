...
# 1. 전체 제목과 부제목의 y 좌표를 그래프 영역에 가깝게 내림
fig.suptitle('Market Overview', fontsize=22, fontweight='medium', color=TEXT_COLOR, y=0.985)
fig.text(0.5, 0.970, f'Korean Listed US ETFs 3-Year Trend ({today_str})', 
...
for ax, (name, ticker) in zip(axes, tickers.items()):
    ...
    # 2. 범례를 그래프 내부 하단 중앙(loc='lower center')에 한 줄(ncol=3)로 배치
...
# 전체 여백 조정: rect의 상단 여백(0.965)을 늘려 그래프 영역을 제목 바로 아래까지 확장
plt.tight_layout(rect=[0, 0.02, 1, 0.965], h_pad=3.5)
