...
# 조회할 티커 심볼 (다우존스는 두 줄로 표시되도록 줄바꿈 적용)
tickers = {
...
# 1. 상단 여백 공간을 줄이기 위해 y값 조정
fig.suptitle('Market Overview', fontsize=22, fontweight='medium', color=TEXT_COLOR, y=0.985)
fig.text(0.5, 0.962, f'Korean Listed US ETFs 3-Year Trend ({today_str})', 
...
for ax, (name, ticker) in zip(axes, tickers.items()):
    ...
    # 개별 차트 제목 설정 (다우존스는 두 줄 높이에 맞추어 pad 조정)
    ax.set_title(name, fontsize=12, fontweight='normal', color=TEXT_COLOR, pad=10, loc='center', linespacing=1.3)
    
    # 범례 설정
    lines = line1 + line2 + line3
    labels = [l.get_label() for l in lines]
    ax.legend(lines, labels, loc='upper right', bbox_to_anchor=(1.0, 1.12), frameon=False, fontsize=8, labelcolor=SUB_TEXT_COLOR, ncol=2)
    ...
    # X축 날짜 3개월 간격 설정 및 35도 기울이기
    ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m'))
    ax.xaxis.set_major_locator(mdates.MonthLocator(interval=3))
    
    # 3. X축 눈금 레이블을 35도 회전하고 우측 정렬
    plt.setp(ax.get_xticklabels(), rotation=35, ha='right')

# 전체 여백 타이트하게 조정 (상단 여백 rect탑을 0.94로 올려 붉은 공간 축소)
plt.tight_layout(rect=[0, 0.02, 1, 0.94], h_pad=4.5)
